<template>
  <div id="app" class="contain">
    <GameStateStart v-if="uiState === 'start'">
      ...
      <p
        v-for="option in characterChoice"
        :key="option"
        calss="character-choices"
      >
        <input
          type="radio"
          :id="option"
      ...
      <button @click="handle">Pick your character!</button>
    </GameStateStart>
    <GameFinish v-else-if="uiState === 'finish'"> </GameFinish>
    <section v-else>
      <!-- base svg -->
      ...
      <div class="zombietalk">
        <p v-for="index in characterChoice" :key="index">
          <button @click="Verify(index)">
...
export default {
  ...
  data() {
    return {
      // todo ruturn this to call global state
      characterInput: '',
    }
  },
  methods: {
    handle() {
      // todo 1 tell store game start!
      // todo 2 tell store game start with which character!
      this.$store.commit('pickCharacter', this.characterInput)
      this.$store.commit('updateUiState', 'uiSate2GS')
    },
    Verify(that) {
      //todo 1 pass next question
      //todo 2 set the score state
      this.$store.commit('updateScore', that === this.character)
