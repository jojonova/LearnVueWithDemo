<template>
  <main class="container cart">
    <div v-if="!totalPrice" class="emptycart">
      <h2>Your Cart</h2>
      <svg
        height="100px"
        width="100px"
      ...
      <nuxt-link to="/"><button>Feed it up</button></nuxt-link>
    </div>
    <div v-else>
      <table>
        <thead>
        ...
        <tbody>
          <tr v-for="(order, index) in cart" :key="index">
            <td>
              {{ order.item }}
              <span v-if="order.options">- {{ order.options }}</span>
            </td>
            <td>
              <span class="comma">{{ order.addOns.join(", ") }}</span>
            </td>
            <td class="price">{{ order.price }}</td>
            <td>
              <button @click="RemoveOneOfFormCart(order)">-</button>
              {{ order.count }}
              <button @click="AddOneOfFormCart(order)">+</button>
            </td>
            <td>{{ order.count * order.price }}</td>
            <td>
              <button class="cancel" @click="CancelOneFormCart(order)">
                X
              </button>
            </td>
          </tr>
...
export default {
  ...
  methods: {
    RemoveOneOfFormCart(item) {
      this.$store.commit("removeOneOfFormCart", item);
    },
    AddOneOfFormCart(item) {
      this.$store.commit("addOneOfFormCart", item);
    },
    CancelOneFormCart(item) {
      this.$store.commit("cancelOneFormCart", item);
    },
  },
};
</script>
